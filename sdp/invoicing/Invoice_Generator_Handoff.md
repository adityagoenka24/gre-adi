...
**Phase 7 — Invoice History added (2 September 2026)**
...
**Phase 8 — Invoice History linked to a JSON file (2 September 2026) — later reverted, see Phase 10**
You asked whether the History log could also live in a plain JSON file you keep somewhere (e.g. the root of the `invoicing` folder), instead of only inside the browser. Added an optional **file link**: at the top of the History tab, a status line shows whether a JSON file is linked, with a **Link JSON File...** button. Clicking it opens your browser's normal "Save File" dialog — pick or create e.g. `invoice_history.json` wherever you like. Once linked, the page automatically reads that file when it loads and writes the latest history back to it every time an invoice is downloaded or a history entry is deleted, keeping the file and the in-browser log in sync (if both ever hold different invoices — say, from being used on two computers — they're merged rather than one overwriting the other). This uses your browser's File System Access API, which **only Chrome and Edge support** — Safari and Firefox will show "This browser can't link a JSON file directly" and the History tab keeps working exactly as in Phase 7, browser-only. Even in Chrome/Edge, the browser will occasionally ask you to **Reconnect** (a one-click security check, not something that can be skipped) — most often the first time you reopen the page in a new browser session. An **Unlink** button is there if you want to stop syncing to a file, or switch to a different one. Nothing about Phases 1-7 changed — this sits on top of the same in-browser history, as a purely additive backup/sync layer.

**Phase 9 — GitHub Sync added (2 September 2026) — later reverted, see Phase 10**
You asked whether the JSON file could instead live at the root of the GitHub repo you use for GitHub Pages hosting, so it's automatic without you managing a local file. GitHub Pages itself can't accept writes (it only serves static files), so this works by having the page talk directly to GitHub's own API. In the History tab, below the local file-link box, there's now a **GitHub Sync** box with a **Configure GitHub Sync** button. Click it and fill in: the repository as `owner/repo`, the branch (defaults to `main`), the path to the JSON file (defaults to `invoice_history.json`), and a GitHub access token. Once saved, the page reads that file from your repo on every load, merges it with what's in the browser (newest entry wins if the two ever differ), and writes the result back to GitHub every time you download a PDF or delete a history entry — each write shows up as a new commit in your repo.

**Important — the access token is a real credential, treat it like a password.** It is stored only in this browser's local storage; it is never written into this HTML page or into the JSON file, so it's never committed to your repo and no one else who opens your public Pages site can see it or use it. But because GitHub Pages is a static site with no server of its own, this is genuinely the only way a webpage can push changes to your repo — there's no way to hide a secret that only your own browser holds from the person actually using that browser. Two things to know: (1) **make the token as narrow as possible** — a fine-grained token, scoped to only this one repository, with only the "Contents: Read and write" permission and nothing else (create one at GitHub -> Settings -> Developer settings -> Personal access tokens -> Fine-grained tokens -> Generate new token); a broader token would be a much bigger loss if it ever leaked. (2) Since your repo is public, the invoice history file becomes publicly readable at your Pages URL like any other file there — anyone who has or finds that URL could see client names, dates, and amounts. If that's ever a problem, the fix is switching the repo to private (needs a paid GitHub plan for Pages) or turning GitHub Sync off (Disconnect button) and relying on the local file link (Phase 8) instead, which never leaves your computer.

**Phase 10 — Simplified back to manual JSON backup/restore (2 September 2026, current)**
Phases 8 and 9 above turned out to be more machinery than you wanted. At your request, both were fully removed — no more local file auto-link, no more GitHub Sync, no token, no automatic reading from anywhere. What replaced them, in the History tab: a **Download JSON** button and an **Upload JSON** button. Download JSON saves a snapshot of everything currently in your History tab to a file — you pick the name and location yourself in your browser's normal save dialog, there's no fixed or required filename. Upload JSON lets you pick any JSON file (again, any filename — nothing checks or restricts what it's called) and merges its invoices into what's already saved in this browser; if an invoice number exists on both sides, whichever copy has the more recent internal timestamp wins, so uploading an old backup can never accidentally erase a newer invoice. Both actions are entirely manual — nothing auto-loads on page open and nothing auto-saves after a download; you decide when to back up or restore. This keeps everything from Phase 7 (the History tab itself, saved automatically to this browser on every Download PDF) exactly as it was — Phase 10 only changes how you get a copy of that history in and out of a file.
...
- The invoice-number counter is stored in that specific browser's local storage — tied to the browser/computer used. If you switch browsers or clear site data, the running number will reset to 01 for the current financial year (just type over it manually if so — nothing else breaks).
- The Invoice History log lives the same way — this browser's local storage on this computer — and nothing writes it anywhere else automatically. Clearing site data, switching browsers, or moving to a different computer will show an empty History tab. Use **Download JSON** beforehand to keep a portable backup, and **Upload JSON** afterward (on the new browser/computer) to bring your invoices back — it merges into whatever's already there rather than replacing it, so it's safe to upload even if some invoices already exist locally.
- The `invoicing/<Client>/` folders (where you move each downloaded PDF) remain the actual PDF record of every invoice regardless of the above — the History tab and its JSON backups are for quickly finding, searching, and re-downloading a past invoice; they don't replace keeping the PDFs themselves.
